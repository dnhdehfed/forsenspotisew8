...
function ProgressBar({ position, duration, onSeek }: {
  ...
  return (
    <div ref={ref} onClick={handleClick} className="w-full h-1 rounded-full cursor-pointer group" style={{background:'var(--border)'}}>
      <div className="h-full rounded-full relative transition-all duration-100" style={{width:`${pct}%`, background:'var(--accent)'}}>
      ...
      {tracks.map((track, i) => {
        ...
        return (
          <button key={`${track.id}-${i}`} onClick={() => onPlay(track, i)}
            ...
            <div className="w-8 h-8 rounded flex-shrink-0 relative overflow-hidden" style={{background:'var(--surface2)'}}>
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm truncate font-medium" style={{color: active ? 'var(--accent)' : 'var(--text)'}}>{track.name}</div>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const onReady = () => {
      ...
      p.addListener('player_state_changed', (state: any) => {
        ...
        if (t) {
          setCurrentTrack({
            id: t.id, name: t.name, uri: t.uri, duration_ms: t.duration_ms,
            artists: t.artists, album: { name: t.album.name, images: t.album.images },
          })
  ...
  const playTrack = useCallback(async (track: Track, trackList: Track[], idx: number) => {
    ...
    await fetch(`https://api.spotify.com/v1/me/player/play?device_id=${deviceId}`, {
      ...
      body: JSON.stringify({ uris: trackList.slice(idx).map(t => t.uri) }),
    })
  }, [deviceId, token])
  ...
  const openPlaylist = async (pl: Playlist) => {
    ...
    try {
      ...
      const items = (data.items || []).map((i: any) => i.track).filter((t: any) => t && t.uri)
      setTracks(items)
    } finally {
  ...
  const seek = async (ms: number) => { setPosition(ms); await player?.seek(ms) }
  const setVol = async (v: number) => { setVolume(v); await player?.setVolume(v) }
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{background:'var(--bg)'}}>
      <div className="flex items-center gap-4 px-6 py-4 border-b" style={{borderColor:'var(--border)'}}>
        <span className="syne font-bold text-lg tracking-tight" style={{color:'var(--accent)'}}>Xaviertify</span>
        <div className="flex gap-1 ml-4">
          {(['playlists','search'] as const).map(v => (
            <button key={v} onClick={() => setView(v)}
              className="px-4 py-1.5 rounded-full text-sm font-medium transition-all"
              style={{background: view === v ? 'var(--accent)' : 'var(--surface)', color: view === v ? '#000' : 'var(--muted)'}}>
        ...
        {playerError && <div className="ml-auto text-xs text-red-400">{playerError}</div>}
        {playerReady && (
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-64 flex-shrink-0 border-r overflow-y-auto py-3" style={{borderColor:'var(--border)'}}>
          {view === 'search' ? (
            ...
            <div>
              <div className="px-4 mb-2 text-xs font-semibold uppercase tracking-widest" style={{color:'var(--muted)'}}>Playlists</div>
              {playlists.map(pl => (
                <button key={pl.id} onClick={() => openPlaylist(pl)}
                  ...
                  <span className="text-sm truncate" style={{color: selectedPlaylist?.id === pl.id ? 'var(--accent)' : 'var(--text)'}}>
                    {pl.name}
                  </span>
                </button>
              ))}
        ...
        <div className="flex-1 overflow-y-auto" style={{animation:'slide-up 0.3s ease'}}>
          {view === 'search' ? (
            <div className="p-4">
              {searching && <div className="flex justify-center pt-8"><Spinner /></div>}
              ...
              {!query && <div className="text-center pt-16 text-sm" style={{color:'var(--muted)'}}>Type to search</div>}
              <TrackList tracks={searchResults} currentTrack={currentTrack} isPlaying={isPlaying}
                onPlay={(t, i) => playTrack(t, searchResults, i)} />
            </div>
          ) : (
            <div className="p-4">
              {!selectedPlaylist && <div className="text-center pt-16 text-sm" style={{color:'var(--muted)'}}>Select a playlist</div>}
              {selectedPlaylist && (
                <>
                  <div className="flex items-end gap-5 mb-6 pb-4 border-b" style={{borderColor:'var(--border)'}}>
                    {selectedPlaylist.images?.[0]?.url && (
                      <img src={selectedPlaylist.images[0].url} className="w-24 h-24 rounded-lg shadow-lg object-cover" alt="" />
                    )}
                    <div>
                  ...
                  <TrackList tracks={tracks} currentTrack={currentTrack} isPlaying={isPlaying}
      ...
      <div className="border-t px-6 py-3" style={{borderColor:'var(--border)', background:'rgba(10,10,10,0.95)', backdropFilter:'blur(20px)'}}>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 w-56 flex-shrink-0">
            ...
            {currentTrack && (
              <div className="min-w-0">
                ...
                <div className="text-xs truncate" style={{color:'var(--muted)'}}>{currentTrack.artists.map((a: any) => a.name).join(', ')}</div>
              </div>
            )}
          </div>
          <div className="flex-1 flex flex-col items-center gap-1.5">
            <div className="flex items-center gap-4">
              <button onClick={skipPrev} className="transition-opacity hover:opacity-100 opacity-60" style={{color:'var(--text)'}}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><polygon points="19,20 9,12 19,4"/><line x1="5" y1="4" x2="5" y2="20" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
              </button>
              <button onClick={togglePlay}
                className="w-10 h-10 rounded-full flex items-center justify-center transition-transform hover:scale-105"
                style={{background:'var(--accent)'}}>
                <span style={{color:'#000', display:'flex', alignItems:'center'}}>
                  {isPlaying ? <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/></svg> : <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><polygon points="5,3 19,12 5,21"/></svg>}
                </span>
              </button>
              <button onClick={skipNext} className="transition-opacity hover:opacity-100 opacity-60" style={{color:'var(--text)'}}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><polygon points="5,4 15,12 5,20"/><line x1="19" y1="4" x2="19" y2="20" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
              </button>
            </div>
            <div className="flex items-center gap-2 w-full max-w-md">
              <span className="text-xs tabular-nums" style={{color:'var(--muted)', minWidth:'36px', textAlign:'right'}}>{fmt(position)}</span>
              <ProgressBar position={position} duration={duration} onSeek={seek} />
              <span className="text-xs tabular-nums" style={{color:'var(--muted)', minWidth:'36px'}}>{fmt(duration)}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 w-32 flex-shrink-0">
            ...
            <input type="range" min={0} max={1} step={0.01} value={volume}
              ...
              className="w-full cursor-pointer h-1" style={{accentColor:'var(--accent)'}} />
          </div>
        </div>
      </div>
    </div>
